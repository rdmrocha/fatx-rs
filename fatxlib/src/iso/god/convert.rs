...
pub fn convert_iso<'a>(
    ...
    if matches!(opts.trim, TrimMode::Compact) {
        ...
        let block_count = compact.data_size().div_ceil(god::BLOCK_SIZE);
        ...
        for part_index in 0..part_count {
            ...
            let remaining_bytes = part_payload_bytes(compact.data_size(), part_index);
    ...
    T: Read + Seek + Write,
{
    if matches!(opts.trim, TrimMode::Compact) {
        let compact = build_compact_source(source_iso, opts.should_abort)?;
        let block_count = compact.data_size().div_ceil(BLOCK_SIZE);
